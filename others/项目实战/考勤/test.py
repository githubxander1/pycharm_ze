...
def process_next_day(time_str):
    if '次日' in str(time_str):
        time_part = str(time_str).split('次日')[1].strip()
        return time_part, '是'
    return str(time_str), '否'

# 应用处理函数并拆分结果
df['下班1打卡时间'], df['是否次日'] = zip(*df['下班1打卡时间'].apply(lambda x: process_next_day(x)))
...
df['下班1打卡时间'] = pd.to_datetime(df['下班1打卡时间'], format='%H:%M', errors='coerce').dt.time
...
def calculate_hours(row):
    ...
    start_dt = datetime.combine(row['日期'], start)
    end_dt = datetime.combine(row['日期'], end)
    work_duration = (end_dt - start_dt).total_seconds() / 3600

    # 计算午休时间
    lunch_duration = 0
    if start < lunch_start and end > lunch_end:
        lunch_duration = (datetime.combine(row['日期'], lunch_end) - datetime.combine(row['日期'], lunch_start)).total_seconds() / 3600
    elif start < lunch_start and end < lunch_end:
        lunch_duration = (datetime.combine(row['日期'], end) - datetime.combine(row['日期'], lunch_start)).total_seconds() / 3600
    elif start >= lunch_start and end > lunch_end:
        lunch_duration = (datetime.combine(row['日期'], lunch_end) - datetime.combine(row['日期'], start)).total_seconds() / 3600

    # 调整工作时长
    adjusted_work_duration = max(0, work_duration - lunch_duration)
